...
p <- 
  ...
  #ggplot(data=df_sr, aes(df_sr$num_students)) +
  #geom_histogram(stat="identity") + 
  scale_y_continuous(name = "Count", labels = scales::comma) +  # unit_format(unit = "K") +
  scale_x_continuous(name="Number of Students") +
  coord_cartesian(xlim = c(0, 60)) +
  theme(panel.grid.minor.y = element_blank()) + 
  theme(panel.background = element_blank()) +
  ggtitle("Students Per Presentation") + 
  theme(text = element_text(size = 14)) #+ 
 # theme(axis.text.x = element_text(angle = 45))
print(p)
...
p <- 
  ...
  #geom_histogram(stat="identity") + 
  scale_y_continuous(name = "Count", labels = scales::comma) +  # unit_format(unit = "K") +
  scale_x_continuous(name="Number of Students: Zoomed into small numbers", breaks =c(1:10)) +
