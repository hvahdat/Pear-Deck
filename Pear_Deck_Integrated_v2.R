...
THEN 1 ELSE 0 END)   AS teacher_event_prop,
...
THEN 1 ELSE 0 END)   AS student_event_prop,
...
THEN 1 ELSE 0 END) AS before_session_prop,
...
THEN 1 ELSE 0 END) AS during_session_prop,
...
THEN 1 ELSE 0 END) AS after_session_prop,
...
for (k in 1:2) {
  ...
  # Turn all to lower case (some inconsistency here) IN BOTH DFS WITH SUBSCRIPTION INFO
  df_statuses$subscription <- tolower(df_statuses$subscription)
  ...
  levels(df_sr$num_students_label) <- paste("prop_stu_", substr(levels(df_sr$num_students_label), 3, nchar(levels(df_sr$num_students_label))), sep="")
  ...
  sql_string <- paste(
    ...
    CASE 
    WHEN currentlyPremium = false AND (currentlyOnTrial = false OR currentlyOnTrial IS NULL) THEN 'free'
  ...
  df_sr_wide$num_prez_audience <- df_sr_wide$total_presentations-(df_sr_wide$total_presentations*df_sr_wide$prop_stu_Testing)
  df_sr_wide$num_prez_testing <- df_sr_wide$total_presentations*df_sr_wide$prop_stu_Testing
  ...
  #####################
  # CREATE: total_prez_aud_label
  #####################
  
  ### Create column: total_prez_aud_label
  # Initialize column
  ...
  # Rename the levels (note: did it this way to preserve the desired order)
  df_sr_wide$total_prez_aud_label <- factor(df_sr_wide$total_prez_aud_label)
  levels(df_sr_wide$total_prez_aud_label) <- substr(levels(df_sr_wide$total_prez_aud_label), 3, nchar(levels(df_sr_wide$total_prez_aud_label)))
...
# Copy df_wide2 for model customization
model_df <- df_wide2
